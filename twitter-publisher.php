...
function twitter_publisher_init() {
	...
	define("TWITTER_OAUTH_HOST","http://twitter.com");
...
function twitter_publisher_user_profile()
{
    ...
    if(isset($_GET['twipub_oauth_token']))
    {
		update_usermeta($profileuser->ID, 'twipub_oauth_token', $_GET['twipub_oauth_token']);
            ...
            <tr>
                ...
                <td>
                	'.(!empty($profileuser->twipub_screen_name) ? $profileuser->twipub_screen_name : @$_GET['screen_name']).'
                   	<a href="http://rebelic.nl/wp-content/plugins/twitter-publisher/oauth_redirect.php?rebelic_redirect_url='.urlencode(get_bloginfo('url').'/wp-admin/profile.php?user_id='.$profileuser->ID).'"><img src="http://a0.twimg.com/images/dev/buttons/sign-in-with-twitter-l.png" /></a>
                </td>
            </tr>
            <tr>
...
function twitter_publisher_post_tweet($post_id = 0, $tweet_manual = false) {
    ...
    $twitter_username = $options['twipub_screen_name'];
    ...
    if(!empty($twitter_username)) {
        ...
        $connection = new Rebelic_TwitterOAuth(CONSUMER_KEY, CONSUMER_SECRET, $options['twipub_oauth_token'], $options['twipub_oauth_token_secret']);
...
*/
		...
		if($options['debug'])
        	add_post_meta($post_id, 'twipub_debug', 'Main twitter: '.$tweet.' - '. print_r($status, true), false);
    }
    ...
    if($author->twipub_share_on_twitter_too && !$tweet_manual &&
        !empty($author->twitter_username)) {
           ...
           $author_url = twitter_publisher_create_short_url($permalink, 'twitter', 'twitter-publisher-author');
			...
			//now let's Tweet about it!
	        $connection = new Rebelic_TwitterOAuth(CONSUMER_KEY, CONSUMER_SECRET, $author->twipub_oauth_token, $author->twipub_oauth_token_secret);

	        $parameters['status'] = $author_tweet;

			$status = $connection->post('statuses/update', $parameters);

			if($options['debug'])
        		add_post_meta($post_id, 'twipub_debug', 'Author twitter: '.$author_tweet.' - '. print_r($status, true), false);
    }
...
function twitter_publisher_configpage() {
	...
	if(isset($_GET['twipub_oauth_token']))
    {
		$twitter_publisher_options['twipub_oauth_token']		= $_GET['twipub_oauth_token'];
		$twitter_publisher_options['twipub_oauth_token_secret'] = $_GET['twipub_oauth_token_secret'];
		$twitter_publisher_options['twipub_screen_name'] 		= $_GET['twipub_screen_name'];

		update_option('twitter_publisher_options', $twitter_publisher_options);
    }
                ...
                <th><label for="twipub_twitter_username">'.__('Twitter username:', 'twitter_publisher').'</label></th>
                 <td>
                	'.$twitter_publisher_options['twipub_screen_name'].'
                   	<a href="http://rebelic.nl/wp-content/plugins/twitter-publisher/oauth_redirect.php?rebelic_redirect_url='.urlencode(get_bloginfo('url').'/wp-admin/options-general.php?page=twitter-publisher.php').'"><img src="http://a0.twimg.com/images/dev/buttons/sign-in-with-twitter-l.png" /></a>
                </td>
            </tr>
